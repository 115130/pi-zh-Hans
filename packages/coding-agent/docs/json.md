...
将所有会话事件以 JSON 行的形式输出到标准输出。适用于将 pi 集成到其他工具或自定义用户界面。
...
`queue_update` 在队列发生变化时发出完整的挂起引导和跟进队列。`compaction_start` 和 `compaction_end` 涵盖手动和自动压缩。
