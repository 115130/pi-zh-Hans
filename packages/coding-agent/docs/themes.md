...
首次运行时，pi 会检测你的终端背景，并默认为 `dark` 或 `light`。
...
**热重载：** 编辑当前激活的自定义主题文件时，pi 会自动重新加载，实现即时视觉反馈。
...
| 令牌 | 用途 |
|-------|---------|
| `bashMode` | 编辑器在 bash 模式下的边框（`!` 前缀） |

### HTML 导出（可选）

`export` 部分控制 `/export` 的 HTML 输出颜色。如果省略，颜色将继承自 `userMessageBg`。
...
- `0-15`：基础 ANSI 颜色（依赖终端）
- `16-231`：6×6×6 RGB 立方体（`16 + 36×R + 6×G + B`，其中 R,G,B 为 0-5）
- `232-255`：灰度渐变

### 终端兼容性

Pi 使用 24 位 RGB 颜色。大多数现代终端都支持此功能（iTerm2、Kitty、WezTerm、Windows 终端、VS Code）。对于仅支持 256 色的较旧终端，pi 会退回到最接近的近似值。
