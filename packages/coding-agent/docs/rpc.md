...
RPC 模式通过标准输入/输出上的 JSON 协议实现编码代理的无头操作。这对于将代理嵌入其他应用、IDE 或自定义 UI 非常有用。

**对于 Node.js/TypeScript 用户**：如果您正在构建 Node.js 应用程序，请考虑直接使用 `@earendil-works/pi-coding-agent` 的 `AgentSession`，而不是生成子进程。有关 API，请参阅 [`src/core/agent-session.ts`](../src/core/agent-session.ts)。对于基于子进程的 TypeScript 客户端，请参见 [`src/modes/rpc/rpc-client.ts`](../src/modes/rpc/rpc-client.ts)。
...
- **事件**：以 JSON 行形式流式输出到标准输出的代理事件

所有命令都支持可选的 `id` 字段，用于请求/响应的关联。如果提供了 `id`，相应的响应将包含相同的 `id`。
...
特别是，Node 的 `readline` 不兼容 RPC 模式的协议，因为它还会在 `U+2028` 和 `U+2029` 上分割，这些字符在 JSON 字符串中是有效的。
...
向代理发送用户提示。命令响应在提示被接受、排队或处理后发出。接受后，事件会异步继续流式传输。
...
**流式传输期间**：如果代理已经在流式传输，您必须指定 `streamingBehavior` 来对消息进行排队：

```json
{"type": "prompt", "message": "New instruction", "streamingBehavior": "steer"}
```

- `"steer"`：在代理运行时将消息排队。它会在当前助手轮次完成其工具调用之后、下一次 LLM 调用之前被传递。
- `"followUp"`：等待代理完成。只有在代理停止时才传递消息。

如果代理正在流式传输且未指定 `streamingBehavior`，该命令将返回错误。

**扩展命令**：如果消息是扩展命令（例如 `/mycommand`），即使在流式传输期间也会立即执行。扩展命令通过 `pi.sendMessage()` 管理自己的 LLM 交互。

**输入扩展**：技能命令（`/skill:name`）和提示模板（`/template`）在发送/排队之前会被扩展。
...
`success: true` 表示提示已被接受、排队或立即处理。`success: false` 表示提示在接受前被拒绝。接受后的失败通过常规的事件和消息流报告，而不是作为同一请求 ID 的第二个 `response`。

`images` 字段是可选的。每个图片使用 `ImageContent` 格式：`{"type": "image", "data": "base64-encoded-data", "mimeType": "image/png"}`。

#### steer

在代理运行时排队一条转向消息。它会在当前助手轮次完成其工具调用之后、下一次 LLM 调用之前被传递。技能命令和提示模板会被扩展。不允许使用扩展命令（请改用 `prompt`）。
...
有关控制转向消息处理方式的说明，请参见 [set_steering_mode](#set_steering_mode)。

#### follow_up

排队一条后续消息，等代理完成后再处理。只有在代理没有更多工具调用或转向消息时才传递。技能命令和提示模板会被扩展。不允许使用扩展命令（请改用 `prompt`）。
...
有关控制后续消息处理方式的说明，请参见 [set_follow_up_mode](#set_follow_up_mode)。
...
启动一个新会话。可以被 `session_before_switch` 扩展事件处理程序取消。
...
循环到下一个可用的模型。如果只有一个模型可用，则返回 `null` 数据。
...
注意：只有 OpenAI codex-max 模型支持 `"xhigh"`。
...
模式：
- `"all"`：在当前助手轮次完成其工具调用后，传递所有转向消息
- `"one-at-a-time"`：每个完成的助手轮次传递一条转向消息（默认）
...
模式：
- `"all"`：当代理完成时，传递所有后续消息
- `"one-at-a-time"`：每次代理完成传递一条后续消息（默认）
...
`bash` 命令会立即执行并返回一个 `BashResult`。内部会创建一个 `BashExecutionMessage` 并存储在代理的消息状态中。此消息不会发出事件。

当下一个 `prompt` 命令被发送时，所有消息（包括 `BashExecutionMessage`）在发送到 LLM 之前会被转换。`BashExecutionMessage` 被转换为 `UserMessage`，格式如下：
...
这意味着：
1. Bash 输出会包含在下一次提示的 LLM 上下文中，而不是立即包含。
...
`tokens` 包含当前会话状态的助手总用量。`contextUsage` 包含用于压缩和页脚显示的实际当前上下文窗口估计值。

当没有模型或上下文窗口可用时，`contextUsage` 被省略。在压缩之后、直到新的压缩后助手响应提供有效的用量数据之前，`contextUsage.tokens` 和 `contextUsage.percent` 为 `null`。
...
从活动分支上的前一条用户消息创建一个新的分叉。可以被 `session_before_fork` 扩展事件处理程序取消。返回被分叉的消息文本。
...
获取可用的命令（扩展命令、提示模板和技能）。这些命令可以通过 `prompt` 命令以 `/` 前缀调用。
...
事件在代理操作期间以 JSON 行形式流式输出到标准输出。事件**不**包含 `id` 字段（只有响应包含）。
...
在执行期间，`tool_execution_update` 事件会流式传输部分结果（例如，bash 输出到达时）：
...
使用 `toolCallId` 关联事件。`tool_execution_update` 中的 `partialResult` 包含到目前为止累积的输出（不仅仅是增量），这样客户端可以在每次更新时直接替换其显示。
...
如果压缩失败（例如，超出 API 配额），则 `result` 为 `null`，`aborted` 为 `false`，`errorMessage` 包含错误描述。
...
- **对话方法**（`select`、`confirm`、`input`、`editor`）：在标准输出上发出 `extension_ui_request`，并阻塞等待客户端在标准输入上发送带有匹配 `id` 的 `extension_ui_response`。
- **即发即弃方法**（`notify`、`setStatus`、`setWidget`、`setTitle`、`set_editor_text`）：在标准输出上发出 `extension_ui_request`，但不期望响应。客户端可以显示该信息或忽略它。

如果对话方法包含 `timeout` 字段，则当超时到期时，代理端会自动使用默认值解析。客户端不需要跟踪超时。
...
注意：在 RPC 模式下 `ctx.hasUI` 为 `true`，因为对话方法和即发即弃方法通过扩展 UI 子协议是功能性的。
...
提示用户从列表中选择。包含 `timeout` 字段的对话方法包含超时时间（毫秒）；如果客户端未及时响应，代理会自动解析为 `undefined`。
...
发送 `widgetLines: undefined`（或省略它）以清除小部件。`widgetPlacement` 字段可以是 `"aboveEditor"`（默认）或 `"belowEditor"`。RPC 模式仅支持字符串数组；组件工厂会被忽略。
...
仅对对话方法（`select`、`confirm`、`input`、`editor`）发送响应。`id` 必须与请求匹配。
...
解除任何对话框方法。扩展接收到 `undefined`（对于 select/input/editor）或 `false`（对于 confirm）。
...
请参见 [`test/rpc-example.ts`](../test/rpc-example.ts) 获取完整的交互式示例，或 [`src/modes/rpc/rpc-client.ts`](../src/modes/rpc/rpc-client.ts) 获取类型化客户端实现。

有关处理扩展 UI 协议的完整示例，请参见 [`examples/rpc-extension-ui.ts`](../examples/rpc-extension-ui.ts)，该文件与 [`examples/extensions/rpc-demo.ts`](../examples/extensions/rpc-demo.ts) 扩展配合使用。
